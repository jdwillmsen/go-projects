...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/jdwillmsen/go-blockchain/utils"
	"log"
	"net/http"
	"strings"
	"sync"
...
func (bc *Blockchain) ValidProof(nonce int, previousHash [32]byte, transactions []*Transaction, difficultly int) bool {
	zeros := strings.Repeat("0", difficultly)
	guessBlock := Block{0, nonce, previousHash, transactions}
	guessHashStr := fmt.Sprintf("%x", guessBlock.Hash())
	return guessHashStr[:difficultly] == zeros
}
...
func (bc *Blockchain) ResolveConflicts() bool {
	...
	for _, n := range bc.neighbors {
		endpoint := fmt.Sprintf("http://%s/cchain", n)
